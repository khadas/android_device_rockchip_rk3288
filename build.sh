...
usage()
{
    ...
    echo "       example: ./build.sh -U -K -A -u -v userdebug     "
    exit 1
}
BUILD_UBOOT=false
BUILD_KERNEL=false
...
cd kernel && make clean && make ARCH=arm $KERNEL_DEFCONFIG && make ARCH=arm $KERNEL_DTS.img -j24 && cd -
if [ $? -eq 0 ]; then
